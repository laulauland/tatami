...
import type { EdgeBinding, EdgeInterval, EdgeIntervalIndex } from "./revision-graph/types";

/** Recency map: commit_id (hex) -> timestamp_millis when last WC */
export type CommitRecency = Record<string, number>;

export function buildEdgeIntervalIndex(
	edgeBindings: EdgeBinding[],
	commitToRow: Map<string, number>,
): EdgeIntervalIndex {
	const intervals: EdgeInterval[] = [];

	for (let order = 0; order < edgeBindings.length; order++) {
		const binding = edgeBindings[order];
	...
	return {
		byMinRow: [...intervals].sort((a, b) => a.minRow - b.minRow || a.order - b.order),
		byMaxRow: [...intervals].sort((a, b) => a.maxRow - b.maxRow || a.order - b.order),
		edgeCount: intervals.length,
	};
}

function upperBoundMinRow(intervals: EdgeInterval[], row: number): number {
	let low = 0;
	let high = intervals.length;

	while (low < high) {
		const mid = Math.floor((low + high) / 2);
		if (intervals[mid].minRow <= row) {
			low = mid + 1;
		} else {
			high = mid;
		}
...
function lowerBoundMaxRow(intervals: EdgeInterval[], row: number): number {
	let low = 0;
	let high = intervals.length;

	while (low < high) {
		const mid = Math.floor((low + high) / 2);
		if (intervals[mid].maxRow < row) {
			low = mid + 1;
		} else {
			high = mid;
		}
...
export function queryEdgeIntervalIndex(
	index: EdgeIntervalIndex,
	startRow: number,
	endRow: number,
): EdgeBinding[] {
	const minRowEnd = upperBoundMinRow(index.byMinRow, endRow);
	const maxRowStart = lowerBoundMaxRow(index.byMaxRow, startRow);
	const maxRowCandidateCount = index.byMaxRow.length - maxRowStart;
	const visibleIntervals: EdgeInterval[] = [];

	if (minRowEnd <= maxRowCandidateCount) {
		for (let i = 0; i < minRowEnd; i++) {
			const interval = index.byMinRow[i];
			if (interval.maxRow >= startRow) {
				visibleIntervals.push(interval);
			}
		}
	} else {
		for (let i = maxRowStart; i < index.byMaxRow.length; i++) {
			const interval = index.byMaxRow[i];
			if (interval.minRow <= endRow) {
				visibleIntervals.push(interval);
			}
		}
	}

	visibleIntervals.sort((a, b) => a.order - b.order);
	return visibleIntervals.map((interval) => interval.data);
}
