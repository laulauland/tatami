				...
				JjNativeAddon.of({
					getRevisionsJson: (repoPath, limit, revset = null, preset = null) =>
						Effect.try({
							...
							catch: (cause) => new JjNativeAddonError({ operation: "getRevisionsJson", cause }),
						}),
					...
					getRevisionDiffJson: (repoPath, changeId) =>
						Effect.try({
							...
							catch: (cause) => new JjNativeAddonError({ operation: "getRevisionDiffJson", cause }),
						}),
					getChangesBatchJson: (repoPath, changeIds) =>
						Effect.try({
							try: () => addon.getChangesBatchJson(repoPath, changeIds),
							catch: (cause) => new JjNativeAddonError({ operation: "getChangesBatchJson", cause }),
						}),
					getDiffsBatchJson: (repoPath, changeIds) =>
						Effect.try({
							try: () => addon.getDiffsBatchJson(repoPath, changeIds),
							catch: (cause) => new JjNativeAddonError({ operation: "getDiffsBatchJson", cause }),
						}),
					generateChangeIds: (repoPath, count) =>
						Effect.try({
							try: () => addon.generateChangeIds(repoPath, count),
							catch: (cause) => new JjNativeAddonError({ operation: "generateChangeIds", cause }),
						}),
